        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 text-sm">
            <Select
              ...
              onValueChange={(v) => setDate((d) => ({ ...d, year: Number(v) }))}
            >
            ...
            <Select
              ...
              onValueChange={(v) => setDate((d) => ({ ...d, month: Number(v) }))}
            >
      ...
      <Tabs defaultValue="source" className="w-full">
        <div className="overflow-x-auto">
          <TabsList className="mb-6 h-auto p-2 gap-1">
            <TabsTrigger value="monthly" className="text-sm px-3 py-1.5 h-auto">月报视图</TabsTrigger>
            <TabsTrigger value="source" className="text-sm px-3 py-1.5 h-auto">部门概览</TabsTrigger>
            <TabsTrigger value="personal" className="text-sm px-3 py-1.5 h-auto">个人统计</TabsTrigger>
            <TabsTrigger value="assembly" className="text-sm px-3 py-1.5 h-auto">总成车间</TabsTrigger>
            <TabsTrigger value="delivery" className="text-sm px-3 py-1.5 h-auto">交车车间</TabsTrigger>
            <TabsTrigger value="project" className="text-sm px-3 py-1.5 h-auto">项目管理</TabsTrigger>
            <TabsTrigger value="process" className="text-sm px-3 py-1.5 h-auto">工艺质量</TabsTrigger>
            <TabsTrigger value="raw" className="text-sm px-3 py-1.5 h-auto">原始数据</TabsTrigger>
          </TabsList>
        </div>
